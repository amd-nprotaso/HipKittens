...
#ifndef D
constexpr int D = 128; // number of heads
#endif
...
__device__ __forceinline__ void mma_AtB_QK(
    ...
    const bf16_2 (&B)[8][1][4],
    const float2 (&C)[2][8])
{
    #pragma unroll
    for (int n = 0; n < 2; n++) {
        mfma_f32_32x32x16_bf16(D[n], A[0][n], B[0][0], C[n]);
        #pragma unroll
        for (int k = 1; k < 8; k++) {
            mfma_f32_32x32x16_bf16(D[n], A[k][n], B[k][0], D[n]);
        }
    }
}
...
// Single contraction-slice of the OV multiply (accumulate onto o_reg in place).
//   A = v_reg[k] ([4][4] = [n][inner]), B = att_in[k] ([4])
__device__ __forceinline__ void mma_AtB_OV_slice(
    float2 (&D)[4][8],
    const bf16_2 (&A)[4][4],
    const bf16_2 (&B)[4])
{
    #pragma unroll
    for (int n = 0; n < 4; n++) {
        mfma_f32_32x32x16_bf16(D[n], A[n], B, D[n]);
    }
}
...
// ============================================================================
// Transpose: q_reg[1][8][4] -> q_reg_t[8][1][4]
// ============================================================================
__device__ __forceinline__ void transpose_q(
    bf16_2 (&dst)[8][1][4], const bf16_2 (&src)[1][8][4])
{
    #pragma unroll
    for (int i = 0; i < 1; i++)
        #pragma unroll
...
// Copy a single tile (one height slice) float2[8] -> bf16_2[8]
__device__ __forceinline__ void copy_f32_to_bf16_half(
    bf16_2 (&dst)[8], const float2 (&src)[8])
{
    #pragma unroll
    for (int k = 0; k < 8; k++)
        dst[k] = __float22bfloat162_rn(src[k]);
}

// ============================================================================
// Lazy-rescale threshold (matches reference)
//   https://github.com/Dao-AILab/flash-attention SM100 cute kernel
// ============================================================================
constexpr float RESCALE_THRESHOLD = 8.0f;

// Per-lane: is (cur - prev) <= T for this lane's column?
__device__ __forceinline__ int lane_below(float prev, float cur, float T) {
    return (cur - prev) <= T;
}
// Wave-uniform vote: true iff EVERY lane is below threshold.
...
__device__ __forceinline__ void col_sum_accum(const float2 (&data)[2][8], float& prev) {
    float sm = 0.0f;
    #pragma unroll
    for (int i = 0; i < 2; i++)
        #pragma unroll
...
__device__ __forceinline__ void exp2_base(float2 (&data)[8]) {
    #pragma unroll
    for (int k = 0; k < 8; k++) {
        data[k].x = __builtin_amdgcn_exp2f(data[k].x);
        data[k].y = __builtin_amdgcn_exp2f(data[k].y);
    }
}
...
// Version using a pre-hoisted SGPR base srsrc.
// The base srsrc spans the whole K/V tensor; each tile is addressed via a
// wave-uniform scalar offset (SOFF) passed as the buffer load's soffset,
// instead of rebuilding the srsrc per tile.
template<bool is_k>
    ...
    uint32_t lds_base)
{
    ...
    // Per-tile scalar byte offset into the base srsrc, kept wave-uniform in SGPRs.
    const bf16* tile_ptr = global_base + (size_t)tile_idx * KV_BLOCK_SIZE * row_stride;
    uint32_t soff = to_sgpr_u32(static_cast<uint32_t>(
        reinterpret_cast<const char*>(tile_ptr) - reinterpret_cast<const char*>(global_base)));

    // Keep the LDS destination address entirely in SGPRs (via m0), passing a
    // null VGPR pointer. This stops the compiler from holding per-buffer LDS
    // addresses live in VGPRs across the whole kernel.
    ...
    for (int i = 0; i < 2; i++) {
        ...
        llvm_amdgcn_raw_buffer_load_lds(
            srsrc_base, (as3_uint32_ptr)0, BYTES_PER_THREAD,
            offsets[i], soff, 0, 0);
...
// Scale Q in registers by the temperature scale (in place)
__device__ __forceinline__ void scale_q(float2 (&q_fl)[8][4], float scale) {
    #pragma unroll
    for (int j = 0; j < 8; j++)
        #pragma unroll
        for (int l = 0; l < 4; l++) {
...
// Convert Q from float registers to bf16 (q_fl[8][4] -> q_reg[8][8])
__device__ __forceinline__ void convert_q_to_bf16(
    bf16 (&q_reg)[8][8], const float2 (&q_fl)[8][4]) {
    #pragma unroll
    for (int j = 0; j < 8; j++)
        #pragma unroll
        for (int l = 0; l < 4; l++) {
            bf16_2 packed = __float22bfloat162_rn(q_fl[j][l]);
            q_reg[j][2 * l]     = packed.x;
            q_reg[j][2 * l + 1] = packed.y;
        }
}
...
__global__ void attend_ker(const attn_globals g) {
    ...
    const int tile_idx = __builtin_amdgcn_readfirstlane(block_tile_idx * NUM_WARPS + warpid());
    ...
    // Per-warp LDS destination bases, hoisted into SGPRs (wave-uniform).
    constexpr uint32_t LDS_BYTES_PER_WARP = 16 * WARP_SIZE; // BYTES_PER_THREAD * WARP_SIZE
    const uint32_t lds_warp_off = (uint32_t)(warpid() * LDS_BYTES_PER_WARP);
    const uint32_t k_lds_base_0 = __builtin_amdgcn_readfirstlane((uint32_t)reinterpret_cast<uintptr_t>(k_smem_0) + lds_warp_off);
    const uint32_t k_lds_base_1 = __builtin_amdgcn_readfirstlane((uint32_t)reinterpret_cast<uintptr_t>(k_smem_1) + lds_warp_off);
    const uint32_t v_lds_base_0 = __builtin_amdgcn_readfirstlane((uint32_t)reinterpret_cast<uintptr_t>(v_smem_0) + lds_warp_off);
    const uint32_t v_lds_base_1 = __builtin_amdgcn_readfirstlane((uint32_t)reinterpret_cast<uintptr_t>(v_smem_1) + lds_warp_off);
    ...
    // Alias the same storage as a [4][4] view (identity in linear memory) to
    // match the reference's reinterpret_cast and avoid a redundant copy.
    auto& att_block_bf16_in = reinterpret_cast<bf16_2(&)[4][4]>(att_block_bf16);
    ...
    group_load_srsrc<true>(0, k_row_stride, swizzled_offsets_K,
                           k_srsrc_base, k_base, k_lds_base_0);
    ...
    // Scale Q
    scale_q(q_reg_fl, TEMPERATURE_SCALE);

    // Convert float -> bf16
    convert_q_to_bf16(q_reg[0], q_reg_fl);

    // Transpose Q (row_l -> col_l), matching the reference's q_reg_transposed
    transpose_q(q_reg_t, reinterpret_cast<const bf16_2(&)[1][8][4]>(q_reg));

    // Load K[1] into shared, V[0] into shared
    group_load_srsrc<true>(1, k_row_stride, swizzled_offsets_K,
                           k_srsrc_base, k_base, k_lds_base_1);
    group_load_srsrc<false>(0, v_row_stride, swizzled_offsets_V,
                            v_srsrc_base, v_base, v_lds_base_0);
    ...
    group_load_srsrc<true>(2, k_row_stride, swizzled_offsets_K,
                           k_srsrc_base, k_base, k_lds_base_0);
    group_load_srsrc<false>(1, v_row_stride, swizzled_offsets_V,
                            v_srsrc_base, v_base, v_lds_base_1);
    asm volatile("s_waitcnt lgkmcnt(0)");
    ...
    for (int j = 3; j < num_tiles - 1; j += 2) {
        ...
        mma_AtB_QK(att_block[1], k_reg_t, q_reg_t, att_block[1]);
        copy_f32_to_bf16_half(att_block_bf16[0], att_block[0][0]);
        // Finish softmax for QK[even]
        exp2_base(att_block[0][1]);
        col_sum_accum(att_block[0], norm_vec);
        copy_f32_to_bf16_half(att_block_bf16[1], att_block[0][1]);
        ...
        group_load_srsrc<true>(j, k_row_stride, swizzled_offsets_K,
                               k_srsrc_base, k_base, k_lds_base_1);
        ...
        mma_AtB_OV_slice(o_reg, v_reg[0], att_block_bf16_in[0]);
        max_vec = col_max_accum(att_block[1], max_vec_prev);
        sched_barrier_pairs<4, 5, 2>();
        
        int lane_ok = lane_below(max_vec_prev, max_vec, RESCALE_THRESHOLD);
        int all_ok  = wave_all_ok(lane_ok);
        if (__builtin_expect(all_ok, 1)) {
            max_vec = max_vec_prev;
            pending_scale = 0;
        } else {
            scale_vec = max_vec_prev - max_vec;
            scale_vec = __builtin_amdgcn_exp2f(scale_vec);
            mul_col_o(o_reg, scale_vec);
            max_vec_prev = max_vec;
            pending_scale = 1;
        }
        
        mma_AtB_OV_slice(o_reg, v_reg[1], att_block_bf16_in[1]);
        mma_AtB_OV_slice(o_reg, v_reg[2], att_block_bf16_in[2]);
        mma_AtB_OV_slice(o_reg, v_reg[3], att_block_bf16_in[3]);
        ...
        group_load_srsrc<false>(j - 1, v_row_stride, swizzled_offsets_V,
                                v_srsrc_base, v_base, v_lds_base_0);
        load_k_from_shared(k_reg, k_smem_0);
        asm volatile("s_waitcnt lgkmcnt(0)");
        asm volatile("s_waitcnt vmcnt(4)");
        ...
        col_sum_accum(att_block[1], norm_vec);
        ...
        group_load_srsrc<true>(j + 1, k_row_stride, swizzled_offsets_K,
                               k_srsrc_base, k_base, k_lds_base_0);
        ...
        lane_ok = lane_below(max_vec_prev, max_vec, RESCALE_THRESHOLD);
        ...
        group_load_srsrc<false>(j, v_row_stride, swizzled_offsets_V,
                                v_srsrc_base, v_base, v_lds_base_1);
        load_k_from_shared(k_reg, k_smem_1);
        asm volatile("s_waitcnt lgkmcnt(0)");
        asm volatile("s_waitcnt vmcnt(4)");
    ...
    group_load_srsrc<true>(num_tiles - 1, k_row_stride, swizzled_offsets_K,
                           k_srsrc_base, k_base, k_lds_base_1);
    load_v_from_shared(v_reg, v_smem_0);
    asm volatile("s_waitcnt lgkmcnt(0)");
    asm volatile("s_waitcnt vmcnt(4)");
    ...
    group_load_srsrc<false>(num_tiles - 2, v_row_stride, swizzled_offsets_V,
                            v_srsrc_base, v_base, v_lds_base_0);
    load_k_from_shared(k_reg, k_smem_0);
    asm volatile("s_waitcnt lgkmcnt(0)");
    asm volatile("s_waitcnt vmcnt(4)");
    ...
    group_load_srsrc<false>(num_tiles - 1, v_row_stride, swizzled_offsets_V,
                            v_srsrc_base, v_base, v_lds_base_1);
    load_k_from_shared(k_reg, k_smem_1);
    asm volatile("s_waitcnt lgkmcnt(0)");
    asm volatile("s_waitcnt vmcnt(2)");
    ...
    float lse = __logf(norm_vec) + lse_max;
