...
print("\n\nPlain HIP kernel:")
for _ in range(num_warmup):
    out_hip = torch.zeros(B, N, H, D, dtype=dtype, device='cuda', requires_grad=True)
    lse_hip = torch.zeros(B, H, 1, N, dtype=torch.float32, device='cuda', requires_grad=True)
    q = torch.randn(B, N, H, D, dtype=dtype, device='cuda', requires_grad=True)
    k = torch.randn(B, N, H_KV, D, dtype=dtype, device='cuda', requires_grad=True)
    v = torch.randn(B, N, H_KV, D, dtype=dtype, device='cuda', requires_grad=True)
    hip_tk_kernel.dispatch_micro(q, k, v, out_hip, lse_hip)
timings_hip = []
out_hip = torch.zeros(B, N, H, D, dtype=dtype, device='cuda', requires_grad=True)
lse_hip = torch.zeros(B, H, 1, N, dtype=torch.float32, device='cuda', requires_grad=True)
torch.manual_seed(0)
...
for _ in range(num_iters):
    ...
    torch.cuda.synchronize()
    elapsed_time = start_event.elapsed_time(end_event)
    timings_hip.append(elapsed_time)
print(f"{out_hip.dtype=}")
avg_time_hip = sum(timings_hip) / len(timings_hip)
eff_hip = efficiency(flops_ref, avg_time_hip)
print(f"Average execution time: {avg_time_hip:.4f} ms")
print(f"Performance: {eff_hip:.2f} TFLOPS for {B=} {H=} {N=} {D=} {causal=} attention.\n")

print(f"\n Plain HIP vs AITER comparison:")
print("\nO outputs:")
print("HIP: ", out_hip[0, 0, :num_print, 0], "Max:", out_hip.max().item())
print("AITER: ", out_ref[0, 0, :num_print, 0], "Max:", out_ref.max().item())
print("\nLSE outputs:")
print("HIP: ", lse_hip[0, 0, 0, :num_print], "Max:", lse_hip.max().item())
print("AITER: ", lse_ref[0, 0, :num_print], "Max:", lse_ref.max().item())
print("Robustness check:")

o_diff_hip, o_err_cnt_hip, o_total_hip, o_rel_error_hip, o_l2_error_hip, o_cos_hip, o_mask_hip = robustness_check(out_hip, out_ref)
print(f"O: max_abs={o_diff_hip.max().item():.6f}, max_rel={o_rel_error_hip:.4f}, "
    f"rel_l2={o_l2_error_hip:.4f}, cos={o_cos_hip:.6f}, "
    f"errors={o_err_cnt_hip}/{o_total_hip} ({100*o_err_cnt_hip/o_total_hip:.4f}%)")
l_diff_hip, l_err_cnt_hip, l_total_hip, l_rel_error_hip, l_l2_error_hip, l_cos_hip, l_mask_hip = robustness_check(lse_hip, lse_ref.unsqueeze(-1).transpose(-1, -2))
print(f"LSE: max_abs={l_diff_hip.max().item():.6f}, max_rel={l_rel_error_hip:.4f}, "
    f"rel_l2={l_l2_error_hip:.4f}, cos={l_cos_hip:.6f}, "
    f"errors={l_err_cnt_hip}/{l_total_hip} ({100*l_err_cnt_hip/l_total_hip:.4f}%)")

print(f"\n Plain HIP vs HipKittens comparison:")
o_diff_tk, o_err_cnt_tk, o_total_tk, o_rel_error_tk, o_l2_error_tk, o_cos_tk, o_mask_tk = robustness_check(out_hip, out)
print(f"O: max_abs={o_diff_tk.max().item():.6f}, max_rel={o_rel_error_tk:.4f}, "
    f"rel_l2={o_l2_error_tk:.4f}, cos={o_cos_tk:.6f}, "
    f"errors={o_err_cnt_tk}/{o_total_tk} ({100*o_err_cnt_tk/o_total_tk:.4f}%)")
l_diff_tk, l_err_cnt_tk, l_total_tk, l_rel_error_tk, l_l2_error_tk, l_cos_tk, l_mask_tk = robustness_check(lse_hip, lse)
