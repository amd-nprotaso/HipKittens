...
__device__ __forceinline__ void mma_AtB_QK(
    float2 (&D)[1][ATT_WIDTH][8],
    const bf16_2 (&A)[8][1][4],
    const bf16_2 (&B)[ATT_WIDTH][8][4],
    const float2 (&C)[1][ATT_WIDTH][8])
{
    #pragma unroll
...
// ============================================================================
// Single contraction-subtile (S) of O += A^T B. C feeds the accumulator input;
// pass C == D to chain onto an existing partial. Mirrors the reference's
// per-subtile mma_AtB(o_reg, subtile<16>(v_reg, S), subtile<16>(att, S), o_reg).
template<int S>
__device__ __forceinline__ void mma_AtB_OV_sub(
    float2 (&D)[4][ATT_WIDTH][8],
    const bf16_2 (&A)[2][4][4],
    const bf16_2 (&B)[ATT_WIDTH][2][4],
    const float2 (&C)[4][ATT_WIDTH][8])
{
    ...
    for (int w = 0; w < ATT_WIDTH; w++) {
        #pragma unroll
        for (int n = 0; n < 4; n++) {
            mfma_f32_32x32x16_bf16(D[n][w], A[S][n], B[w][S], C[n][w]);
        }
    }
}
...
__device__ __forceinline__ void copy_f32_to_bf16(
    bf16_2 (&dst)[1][ATT_WIDTH][8], const float2 (&src)[1][ATT_WIDTH][8])
{
    #pragma unroll
    for (int i = 0; i < 1; i++)
        #pragma unroll
...
__device__ __forceinline__ void reinterpret_att(
    bf16_2 (&dst)[ATT_WIDTH][2][4], const bf16_2 (&src)[1][ATT_WIDTH][8])
{
    #pragma unroll
    for (int j = 0; j < ATT_WIDTH; j++)
        #pragma unroll
...
__device__ __forceinline__ void col_max_reset(
    float (&mx)[ATT_WIDTH], const float2 (&data)[1][ATT_WIDTH][8]) {
    #pragma unroll
    for (int j = 0; j < ATT_WIDTH; j++) {
...
__device__ __forceinline__ void col_sum_accum(
    float (&sm_out)[ATT_WIDTH], const float2 (&data)[1][ATT_WIDTH][8],
    const float (&prev)[ATT_WIDTH]) {
    #pragma unroll
    for (int j = 0; j < ATT_WIDTH; j++) {
        float sm = 0.0f;
...
__device__ __forceinline__ void sub_col_att(
    float2 (&data)[1][ATT_WIDTH][8], const float (&val)[ATT_WIDTH]) {
    #pragma unroll
    for (int j = 0; j < ATT_WIDTH; j++) {
        #pragma unroll
        for (int i = 0; i < 1; i++)
...
__device__ __forceinline__ void zero_att(float2 (&data)[1][ATT_WIDTH][8]) {
    #pragma unroll
    for (int i = 0; i < 1; i++)
        #pragma unroll
        for (int j = 0; j < ATT_WIDTH; j++)
...
__device__ __forceinline__ void prefill_offsets(
    uint32_t (&offsets)[1], int row_stride)
{
...
__device__ __forceinline__ void group_load_srsrc(
    bf16_2* smem_base,
    int tile_idx, int row_stride,
    const uint32_t (&offsets)[1],
    int32_t s0, int32_t s1)
{
    constexpr int BYTES_PER_THREAD = 16;
    constexpr int BYTES_PER_WARP = BYTES_PER_THREAD * WARP_SIZE;
    constexpr int ELEMS_PER_WARP = BYTES_PER_WARP / sizeof(bf16_2);

    const int wid = warpid();

    // Rebuild the hoisted strided base srsrc {x,y,z,w}.
    // z (range) and w (config) are tile-invariant, reconstruct them here.
    i32x4 srsrc;
    srsrc.x = s0;
    srsrc.y = s1;
    srsrc.z = (int32_t)(uint32_t)(row_stride * ATTN_N * 2);
    srsrc.w = (int32_t)0x110000;
    ...
    bf16_2* lds_base_ptr = smem_base + wid * ELEMS_PER_WARP;

    #pragma unroll
    for (int i = 0; i < 1; i++) {
        bf16_2* lds_elem_ptr = lds_base_ptr + i * NUM_WARPS * ELEMS_PER_WARP;
        uintptr_t lds_addr = reinterpret_cast<uintptr_t>(lds_elem_ptr);
        as3_uint32_ptr lds_ptr = (as3_uint32_ptr)(lds_addr);
...
__device__ __forceinline__ void load_k_from_shared(
    bf16_2 (&k_reg)[1][8][4], const bf16_2* smem)
{
    const int lid = laneid();
    ...
    #pragma unroll
    for (int ii = 0; ii < 1; ii++) {        // subtiles_per_col
        #pragma unroll
...
__device__ __forceinline__ void load_v_from_shared(
    bf16_2 (&v_reg)[2][4][4], const bf16_2* smem)
{
    const int lid = laneid();
    ...
    for (int k = 0; k < 2; k++) { // num_strides / stride_groups_per_shared_subtile_col
        ...
        #pragma unroll
        for (int i = 0; i < 2; i++) { // height = 2 (KV_BLOCK_SIZE/16 contraction subtiles)
...
__global__ void attend_ker(const attn_globals g) {
    ...
    constexpr int KV_TILE_ELEMS_2 = KV_BLOCK_SIZE * ATTN_D / 2;
    bf16_2* k_smem[2];
    bf16_2* v_smem[2];
    k_smem[0] = reinterpret_cast<bf16_2*>(__shm);
    k_smem[1] = k_smem[0] + KV_TILE_ELEMS_2;
    v_smem[0] = k_smem[1] + KV_TILE_ELEMS_2;
    v_smem[1] = v_smem[0] + KV_TILE_ELEMS_2;
    ...
    bf16_2 k_reg[1][8][4];
    bf16_2 k_reg_t[8][1][4];
    bf16_2 v_reg[2][4][4];
    float2 o_reg[4][ATT_WIDTH][8];
    float2 att_block[1][ATT_WIDTH][8]; // single 32x32 tile: [height=1][width][packed]
    bf16_2 att_block_bf16[1][ATT_WIDTH][8];
    bf16_2 att_block_bf16_in[ATT_WIDTH][2][4];
    ...
    for (int w = 0; w < ATT_WIDTH; w++) {
        norm_vec[w] = 0.0f;
        scale_vec[w] = 1.0f;
        max_vec[w] = -__builtin_huge_valf();
        max_vec_prev[w] = -__builtin_huge_valf();
    }

    // Prefill swizzled offsets
    uint32_t swizzled_offsets_K[1];
    uint32_t swizzled_offsets_V[1];
    ...
    // ========================================================================
    // Simple correct double-buffered loop over the 32-row KV tiles.
    // Online softmax: per tile, compute QK = K^T Q, update running max, rescale
    // the running output O and norm, exponentiate, accumulate, then O += V^T A.
    // ========================================================================
    for (int t = 0; t < num_tiles; t++) {
        const int buf = t & 1;

        // Stage K[t], V[t] into shared.
        group_load_srsrc<true>(k_smem[buf], t, k_row_stride, swizzled_offsets_K,
                               ks0, ks1);
        group_load_srsrc<false>(v_smem[buf], t, v_row_stride, swizzled_offsets_V,
                                vs0, vs1);
        ...
        // Shared -> registers.
        load_k_from_shared(k_reg, k_smem[buf]);
        load_v_from_shared(v_reg, v_smem[buf]);
        __builtin_amdgcn_sched_barrier(0);
        ...
        mma_AtB_QK(att_block, k_reg_t, q_reg, att_block);

        // Online max + rescale of running O and norm. The first tile has no
        // running state to rescale (and -ffast-math makes -inf arithmetic
        // unsafe), so reset the max instead of accumulating from -inf.
        if (t == 0) {
            col_max_reset(max_vec, att_block);
        } else {
            col_max_accum(max_vec, att_block, max_vec_prev);
            rv_diff_exp2(scale_vec, max_vec_prev, max_vec); // exp2(prev - cur)
            mul_col_o(o_reg, scale_vec);
            rv_mul(norm_vec, scale_vec);
        }
        rv_copy(max_vec_prev, max_vec);

        // Exponentiate this tile and accumulate the norm.
        sub_col_att(att_block, max_vec);
        exp2_base(att_block[0]);
        col_sum_accum(norm_vec, att_block, norm_vec);

        // Convert to bf16 and reshape for the OV MMA.
        copy_f32_to_bf16(att_block_bf16, att_block);
        reinterpret_att(att_block_bf16_in, att_block_bf16);

        // O += V^T A.
        mma_AtB_OV(o_reg, v_reg, att_block_bf16_in, o_reg);

        // All warps done reading this smem buffer before it is overwritten.
        __builtin_amdgcn_sched_barrier(0);
        __builtin_amdgcn_s_barrier();
        __builtin_amdgcn_sched_barrier(0);
    }
